...
class SanPham1
{
    ...
    static function all()
    {
        ...
        foreach ($req->fetchAll() as $item) {
            $list[] = new SanPham1($item['Id'], $item['MaSanPham'], $item['TenSanPham'],$item['Gia'],
    ...
    static function add($data)
    {
        try {
            $db = DB::getInstance();
            $sql = 'INSERT INTO SanPham1(Id, MaSanPham, TenSanPham, Gia, SoLuong,
    ...
    static function edit($data)
    {
        try {
            $db = DB::getInstance();
            $sql = 'UPDATE SanPham1 SET MaSanPham="' . $data->MaSanPham . '", TenSanPham="' . $data->TenSanPham . '" , Gia="' . $data->Gia . '" ,
    ...
    static function find($id)
    {
        ...
        $item = $req->fetch();
        if (isset($item['Id'])) {
            return new SanPham1($item['Id'], $item['MaSanPham'], $item['TenSanPham'],$item['Gia'],
            $item['SoLuong'],$item['MoTa'],$item['AnhSanPham'],$item['MauSac'],$item['RAM'],
            $item['CPU'],$item['CameraTruoc'],$item['CameraSau'],$item['TheNho'],$item['HeDieuHanh'],$item['ManHinh'],
            $item['DoPhanGiai'],$item['Pin'],$item['BaoHanh'],$item['NhaSanXuatId'],$item['LoaiSanPhamId']
           );
    ...
    static function getByLTS($ltsid)
    {
      ...
      foreach ($req->fetchAll() as $item) {
        $list[] = new SanPham1($item['Id'], $item['TenSanPham'],$item['MaSanPham'],$item['MoTa'],
